...
impl<M: CudaGraphModule> GraphCapturer<M> {
    ...
    pub fn capture_mtp(
        ...
        for phase in CapturePhase::ALL {
            ...
            if phase.is_warmup() {
                let _ = self
                    .model
                    .forward(&input_ids, &positions, kv_caches, &input_metadata)?;
                #[cfg(feature = "cuda")]
                if !should_capture {
                    device.synchronize()?;
                }
    ...
    pub fn replay_mtp(
        ...
        if let Some(b_tables) = input_metadata.block_tables.as_ref() {
            let pad = max_num_blocks.saturating_sub(b_tables.dim(1)?);
            let padded_table = b_tables.pad_with_zeros(1, 0, pad)?.contiguous()?;
        ...
        if let Some(fm) = input_metadata.flashinfer_metadata.as_ref() {
            ...
            let batch_indices = fm.batch_indices.as_ref().ok_or_else(|| {
                candle_core::Error::msg("MTP graph replay requires flashinfer batch_indices")
            })?;
            let flashinfer_positions = fm.positions.as_ref().ok_or_else(|| {
                candle_core::Error::msg("MTP graph replay requires flashinfer positions")
            })?;
            ...
            if let Some(params) = self.flashinfer_kv_params {
                ...
                let kv_len_arr_host = fm.kv_len_arr_host.as_deref().ok_or_else(|| {
                    candle_core::Error::msg("MTP graph replay requires kv_len_arr_host")
                })?;
                let q_cu_seqlens_host = vec![0u32, verify_len as u32];
