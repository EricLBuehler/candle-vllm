...
fn to_openai_tools(tools: &[crate::tools::Tool]) -> Vec<openai_protocol::common::Tool> {
    tools.iter().map(|t| openai_protocol::common::Tool {
...
pub enum ToolModelType {
    LLaMa, Qwen, Qwen3MoE, Mistral, Gemma, Gemma3,
    Phi, Phi4, GLM4, Yi, StableLM, DeepSeek,
}

#[derive(Debug, Clone, PartialEq)]
pub enum ParserState { Normal, Buffering }
...
impl ToolConfig {
    pub fn for_model_type(model_type: &ToolModelType) -> Self {
        ...
        match model_type {
            ToolModelType::LLaMa => {
                ...
                Self { start_token_ids: start_ids, end_token_ids: end_ids,
                    start_token_str: "<|python_tag|>".into(), end_token_str: "<|eom_id|>".into() }
            }
            ToolModelType::Qwen | ToolModelType::Qwen3MoE => {
                start_ids.insert(151657);
                end_ids.insert(151658);
                Self { start_token_ids: start_ids, end_token_ids: end_ids,
                    start_token_str: "<tool_call>".into(), end_token_str: "</tool_call>".into() }
            }
            ToolModelType::Mistral => {
                start_ids.insert(9);
                Self { start_token_ids: start_ids, end_token_ids: end_ids,
                    start_token_str: "[TOOL_CALLS]".into(), end_token_str: "]".into() }
            }
            ToolModelType::Gemma | ToolModelType::Gemma3 => {
                Self { start_token_ids: start_ids, end_token_ids: end_ids,
                    start_token_str: "<start_function_call>".into(), end_token_str: "<end_function_call>".into() }
            }
            _ => Self {
                start_token_ids: HashSet::new(), end_token_ids: HashSet::new(),
                start_token_str: "<tool_call>".into(), end_token_str: "</tool_call>".into(),
            },
        }
    }

    pub fn has_special_tokens(&self) -> bool { self.has_start_tokens() }
    pub fn has_start_tokens(&self) -> bool { !self.start_token_ids.is_empty() }
    pub fn has_end_tokens(&self) -> bool { !self.end_token_ids.is_empty() }
    ...
    pub fn validate_with_tokenizer(&mut self, tokenizer: &Tokenizer, model_type: &ToolModelType) {
        ...
        {
            if Self::try_rebind_single_token_id(tokenizer, &self.start_token_str, &mut self.start_token_ids) {
                tracing::warn!("Tool start token IDs corrected for model {:?}: {:?}", model_type, self.start_token_ids);
            } else {
        ...
        {
            if Self::try_rebind_single_token_id(tokenizer, &self.end_token_str, &mut self.end_token_ids) {
                tracing::warn!("Tool end token IDs corrected for model {:?}: {:?}", model_type, self.end_token_ids);
            } else {
    ...
    pub fn tool_call_end_ids(&self, tokenizer: &Tokenizer) -> Vec<u32> {
        ...
        if self.has_end_tokens() {
            ...
            if use_special { ids.extend(self.end_token_ids.iter().copied()); used_special = true; }
        }
        if !used_special && !self.end_token_str.is_empty() && self.end_token_str.starts_with('<') {
            if let Ok(encoded) = tokenizer.encode(self.end_token_str.as_str(), false) {
                let tok_ids = encoded.get_ids();
                if tok_ids.len() == 1 { ids.push(tok_ids[0]); }
            }
        }
        ids
    }
    ...
    pub fn tool_call_start_ids(&self, tokenizer: &Tokenizer) -> Vec<u32> {
        ...
        if self.has_start_tokens() {
            ...
            if use_special { ids.extend(self.start_token_ids.iter().copied()); used_special = true; }
        }
        if !used_special && !self.start_token_str.is_empty() && self.start_token_str.starts_with('<') {
            if let Ok(encoded) = tokenizer.encode(self.start_token_str.as_str(), false) {
                let tok_ids = encoded.get_ids();
                if tok_ids.len() == 1 { ids.push(tok_ids[0]); }
            }
    ...
    fn matches_single_token(tokenizer: &Tokenizer, text: &str, token_ids: &HashSet<u32>) -> bool {
        ...
        match tokenizer.encode(text, false) {
            Ok(encoded) => { let ids = encoded.get_ids(); ids.len() == 1 && token_ids.contains(&ids[0]) }
            Err(_) => false,
        }
    }

    fn try_rebind_single_token_id(tokenizer: &Tokenizer, text: &str, token_ids: &mut HashSet<u32>) -> bool {
        if text.is_empty() { return false; }
        if let Ok(encoded) = tokenizer.encode(text, false) {
            let ids = encoded.get_ids();
            if ids.len() == 1 { token_ids.clear(); token_ids.insert(ids[0]); return true; }
        }
        if let Some(id) = tokenizer.get_vocab(true).get(text).copied() {
            token_ids.clear(); token_ids.insert(id); return true;
        }
        false
    }
}
...
pub fn detect_prefilled_reasoning_end_marker(prompt: &str) -> Option<String> {
    ...
    for &(start, end) in REASONING_MARKERS {
        if trimmed.ends_with(start) { return Some(end.to_string()); }
    }
    None
}
...
impl StreamToolParser {
    ...
    pub fn new_with_config(
        model_type: &ToolModelType, model_id: String, config: ToolConfig,
        tools: Vec<Tool>, enforce_parser: Option<String>,
    ) -> Self {
        let openai_tools = to_openai_tools(&tools);
        let parse_strategy = match model_type {
            ToolModelType::Mistral => "mistral_list", _ => "json",
        }.to_string();

        let factory = ParserFactory::new();
        let parser_name = if let Some(name) = enforce_parser.as_ref().and_then(|s| {
            let trimmed = s.trim(); if trimmed.is_empty() { None } else { Some(trimmed) }
        }) {
            if !factory.registry().has_parser(name) {
                let valid = factory.list_parsers().join(", ");
                panic!("Invalid enforce-parser '{}'. Valid parsers: {}", name, valid);
            }
            name
        } else {
        ...
        if !tools.is_empty() {
            tracing::info!("Tool parser selected: {} (model_id={}, enforce_parser={})",
                parser_name, model_id, enforce_parser.as_deref().unwrap_or("none"));
        }

        let parser = factory.registry().create_parser(parser_name)
        ...
        Self {
            config, state: ParserState::Normal, buffer: String::new(),
            model_id, parse_strategy, parser, tools: openai_tools,
            streaming_calls: Vec::new(), accumulated_output: String::new(),
            active_reasoning_end: None, in_code_block: false,
            saw_buffer_parse_activity: false, buffer_had_parse_activity: false,
    ...
    pub fn in_reasoning(&self) -> bool { self.active_reasoning_end.is_some() }
    pub fn set_initial_reasoning_end_marker(&mut self, end_marker: Option<String>) {
        self.active_reasoning_end = end_marker;
    }
    pub fn in_code_block(&self) -> bool { self.in_code_block }
    pub fn state(&self) -> &ParserState { &self.state }
    pub fn accumulated_output(&self) -> &str { &self.accumulated_output }
    ...
    pub fn process_token(&mut self, token_id: u32, token_text: &str) -> StreamResult {
        ...
        for line in self.accumulated_output.lines() {
            if line.trim().starts_with("```") { code_block_count += 1; }
        }
        ...
        match self.state.clone() {
            ParserState::Normal => {
                ...
                if self.is_start_token(token_id, token_text) {
                    ...
                    tracing::info!("Tool call {} ({}) found, start buffering!", token_text, token_id);
            ...
            ParserState::Buffering => {
                ...
                if nested_start {
                    tracing::warn!("Ignoring nested tool-call start marker while buffering: {:?}", token_text);
                } else {
                    if let Ok(result) = futures::executor::block_on(
                        self.parser.parse_incremental(token_text, &self.tools)
                    ) {
                ...
                if end_reached {
                    ...
                    self.pending_end_marker_candidate = false;
                    tracing::info!("Tool call buffering end, reached {} ({})", token_text, token_id);
                    ...
                    let result = if tool_calls.is_empty() {
                        if had_partial {
                            tracing::warn!("End marker seen but tool call still incomplete; continuing");
                    ...
                    if matches!(result, StreamResult::Buffering) { return result; }
                    self.parser.reset();
                    self.buffer.clear();
    ...
    pub fn finalize_buffered_tool_calls(&mut self) -> Option<BufferedFinalizeResult> {
        ...
        if tool_calls.is_empty() || (!strict_complete && !recoverable) {
            ...
            tracing::warn!("Recovered {} tool call(s) from buffered state at stream end", tool_calls.len());
    ...
    pub async fn parse_complete_with_fallback(&self, text: &str) -> Vec<ToolCall> {
        let mut parsed_calls = match self.parser.parse_complete(text).await {
            ...
            Err(err) => { tracing::warn!("Tool parse failed: {:?}", err); Vec::new() }
        };
        ...
        if parsed_calls.is_empty()
            ...
            && (text.contains(&self.config.start_token_str) || text.contains(&self.config.end_token_str))
        {
        ...
        if parsed_calls.is_empty() && self.parse_strategy == "json" && self.parser.has_tool_markers(text) {
            let factory = ParserFactory::new();
            if let Some(json_parser) = factory.registry().create_parser("json") {
                if let Ok((normal_text, calls)) = json_parser.parse_complete(text).await {
                    if normal_text.trim().is_empty() { parsed_calls = calls; }
                }
            }
        }

        parsed_calls.into_iter().map(crate::tools::tool_call_from_parser).collect()
    }

    // ── Private helpers ──────────────────────────────────────────────────

    fn is_start_token(&self, id: u32, _text: &str) -> bool {
        if self.config.has_start_tokens() { return self.config.start_token_ids.contains(&id); }
        let current_line = self.accumulated_output.rsplit('\n').next().unwrap_or("");
        let candidate = current_line.trim_start_matches(|c| c == ' ' || c == '\t' || c == '\r');
        if candidate.starts_with(&self.config.start_token_str) { return true; }
        let min_prefix = Self::safe_partial_prefix_len(&self.config.start_token_str);
        !candidate.is_empty() && candidate.len() >= min_prefix
            && self.config.start_token_str.starts_with(candidate)
    }

    fn is_end_token(&self, id: u32, text: &str) -> bool {
        if self.config.has_end_tokens() { return self.config.end_token_ids.contains(&id); }
        if self.parse_strategy == "mistral_list" && self.config.end_token_str == "]" { return false; }
        text.contains(&self.config.end_token_str)
    }

    fn apply_streaming_result(&mut self, result: &StreamingParseResult) {
        if !result.calls.is_empty() { self.apply_stream_items(&result.calls); }
    }

    fn apply_stream_items(&mut self, items: &[ToolCallItem]) {
        if !items.is_empty() { self.buffer_had_parse_activity = true; }
        for item in items {
            if self.streaming_calls.len() <= item.tool_index {
                self.streaming_calls.resize_with(item.tool_index + 1, StreamingToolCallState::default);
            }
            let state = &mut self.streaming_calls[item.tool_index];
            if let Some(name) = &item.name { state.name = Some(name.clone()); }
            if !item.parameters.is_empty() { state.arguments.push_str(&item.parameters); }
        }
    }
    ...
    fn build_tool_calls_from_streaming(&mut self) -> Vec<ToolCall> {
        ...
        for state in &self.streaming_calls {
            ...
            calls.push(crate::tools::new_tool_call(
                crate::tools::generate_tool_call_id(), name.clone(), args,
            ));
        }
        calls
    }
    ...
    async fn has_strict_complete_tool_call(&self) -> bool {
        if !self.has_complete_tool_envelope() { return false; }
        if self.streaming_calls.iter().any(|c| c.name.is_none()) { return false; }
        if !self.streaming_calls.is_empty()
            && self.streaming_calls.iter().all(|c| c.arguments.trim().is_empty())
        { return true; }
        if !self.streaming_calls.is_empty()
            && self.streaming_calls.iter().all(|c| serde_json::from_str::<Value>(c.arguments.trim()).is_ok())
        { return true; }
        !self.parse_complete_with_fallback(&self.buffer).await.is_empty()
    }

    fn can_recover_incomplete_buffered_tool_calls(&self) -> bool {
        if self.streaming_calls.is_empty() { return false; }
        ...
        self.streaming_calls.iter().all(|c| {
            ...
            args.is_empty() || serde_json::from_str::<Value>(&self.finalize_streamed_arguments(args)).is_ok()
        })
    }

    fn has_ambiguous_incomplete_end_marker(&self) -> bool {
        if self.config.end_token_str.is_empty() || !self.config.end_token_str.starts_with('<') { return false; }
        self.buffer.contains(&self.config.end_token_str) && !self.has_complete_tool_envelope()
    }

    fn has_complete_tool_envelope(&self) -> bool {
        if !self.config.start_token_str.starts_with('<') || !self.config.end_token_str.starts_with('<') {
            return true;
        }
        let Some(start_idx) = self.buffer.find(&self.config.start_token_str) else { return true; };
        let section = &self.buffer[start_idx..];
        let Some(end_rel) = section.rfind(&self.config.end_token_str) else { return false; };
        ...
        if inner_end < inner_start { return false; }
        let inner = self.buffer[inner_start..inner_end].trim();

        if block.contains("<function=") || block.contains("<parameter=") {
            let Some(fs) = block.find("<function=") else { return false; };
            let func_section = &block[fs..];
            let Some(fe_rel) = func_section.rfind("</function>") else { return false; };
    ...
    fn has_balanced_parameter_tags(function_block: &str) -> bool {
        ...
        while idx < function_block.len() {
            ...
            match (open_pos, close_pos) {
                (None, None) => break,
                (Some(op), None) => { open_count += 1; idx = op + OPEN.len(); }
                (None, Some(cp)) => { if open_count > 0 { open_count -= 1; } idx = cp + CLOSE.len(); }
                (Some(op), Some(cp)) => {
                    if op < cp { open_count += 1; idx = op + OPEN.len(); }
                    else { if open_count > 0 { open_count -= 1; } idx = cp + CLOSE.len(); }
                }
            }
        }
        open_count == 0
    ...
    fn finalize_streamed_arguments(&self, raw: &str) -> String {
        ...
        if serde_json::from_str::<Value>(trimmed).is_ok() { return trimmed.to_string(); }
    ...
    fn buffer_has_end_tag(&self) -> bool {
        if self.config.end_token_str.is_empty() { return false; }
        if self.config.has_end_tokens() { return false; }
        if self.parse_strategy == "mistral_list" && self.config.end_token_str == "]" { return false; }
        self.buffer.contains(&self.config.end_token_str)
    }

    fn maybe_complete_mistral_list(&self) -> bool {
        if self.parse_strategy != "mistral_list" { return false; }
    ...
    fn parser_name_for_model(model_type: &ToolModelType, model_id: &str) -> &'static str {
        ...
        match model_type {
            ...
            ToolModelType::Qwen | ToolModelType::Qwen3MoE => {
                if model_lower.contains("coder") || model_lower.contains("qwen3.5") { "qwen_coder" }
                else { "qwen" }
            }
    ...
    fn safe_partial_prefix_len(start_tag: &str) -> usize {
        if let Some(idx) = start_tag.find('_') { return idx.max(2); }
        start_tag.find('>').map_or(6, |idx| idx).clamp(2, 6)
    }

    fn escape_marker_for_display(marker: &str) -> String {
        if let Some(rest) = marker.strip_prefix('<') { format!("<\u{200C}{}", rest) }
        else { format!("{}\u{200C}", marker) }
    }

    fn contains_partial_marker_fragment(text: &str, marker: &str) -> bool {
        if marker.is_empty() || !Self::should_escape_marker_for_display(marker) || !marker.starts_with('<') {
            return false;
        }
        let marker_len = marker.len();
        if marker_len < 4 { return false; }
        let min_prefix = Self::safe_partial_prefix_len(marker).min(marker_len.saturating_sub(1));
        if min_prefix >= marker_len { return false; }
        (min_prefix..marker_len).rev().any(|len| text.contains(&marker[..len]))
    }
    ...
    fn escape_partial_marker_fragments(text: &str, marker: &str) -> String {
        ...
        let marker_len = marker.len();
        if marker_len < 4 { return text.to_string(); }
        let min_prefix = Self::safe_partial_prefix_len(marker).min(marker_len.saturating_sub(1));
        if min_prefix >= marker_len { return text.to_string(); }
        let mut out = text.to_string();
    ...
    fn should_escape_marker_for_display(marker: &str) -> bool {
        if marker.is_empty() || marker.len() < 3 { return false; }
    ...
    fn display_escape_markers(&self) -> Vec<String> {
        ...
        for marker in [&self.config.start_token_str, &self.config.end_token_str] {
            if Self::should_escape_marker_for_display(marker) { markers.push(marker.to_string()); }
        }
        if self.config.start_token_str.contains("tool_call") && self.config.end_token_str.contains("tool_call") {
            markers.extend(["<function=", "</function>", "<parameter=", "</parameter>"]
    ...
    fn recover_streaming_arguments_from_buffer(&mut self) {
        if self.streaming_calls.is_empty() || !self.buffer.contains("<parameter=") { return; }
        for state in &mut self.streaming_calls {
            let Some(name) = state.name.as_deref() else { continue };
            ...
            let mut args_obj = match serde_json::from_str::<Value>(state.arguments.trim()) {
                Ok(Value::Object(map)) => map, _ => Map::new(),
            };
    ...
    fn extract_xml_parameters_for_function(buffer: &str, function_name: &str) -> std::collections::HashMap<String, String> {
        ...
        let Some(func_start) = buffer.rfind(&function_tag).or_else(|| buffer.rfind(&alt_function_tag)) else {
            return recovered;
        };
        ...
        while let Some(rel) = section[cursor..].find(PARAM_PREFIX) {
            ...
            let Some(name_end_rel) = section[name_start..].find('>') else { break };
            let name_end = name_start + name_end_rel;
            let parameter_name = section[name_start..name_end].trim().trim_matches('"').trim_matches('\'').to_string();
            if parameter_name.is_empty() { break; }
            let value_start = name_end + 1;
            if value_start > section.len() { break; }
            if let Some(value_end_rel) = section[value_start..].find(PARAM_END) {
                let value_end = value_start + value_end_rel;
                let value = section[value_start..value_end].trim_matches(|c| c == '\n' || c == '\r').to_string();
                recovered.insert(parameter_name, value);
                cursor = value_end + PARAM_END.len();
            } else {
                let value = section[value_start..].trim_matches(|c| c == '\n' || c == '\r').to_string();
                recovered.insert(parameter_name, value);
                break;
            }
        }
...
fn repair_streamed_json_arguments(raw: &str) -> String {
    ...
    if repaired.is_empty() { return "{}".to_string(); }
    ...
    for ch in repaired.chars() {
        if in_string {
            ...
            match ch { '\\' => escaped = true, '"' => in_string = false, _ => {} }
            continue;
        }
        match ch {
            '"' => in_string = true,
    ...
    while repaired.chars().last().is_some_and(|c| c.is_whitespace() || c == ',') { repaired.pop(); }
    while let Some(open) = stack.pop() {
        repaired.push(match open { '{' => '}', '[' => ']', _ => continue });
    }
    repaired
}
...
mod tests {
    ...
    fn test_parser_normal_content() {
        ...
        let mut parser = StreamToolParser::new_with_config(
            &ToolModelType::Qwen, "qwen3".into(),
            ToolConfig::for_model_type(&ToolModelType::Qwen), tools, None,
        );
    ...
    fn test_sanitize_tool_markup_escapes_xml() {
        ...
        let parser = StreamToolParser::new_with_config(
            &ToolModelType::Qwen, "qwen3-coder".into(),
            ToolConfig::for_model_type(&ToolModelType::Qwen), tools, None,
        );
