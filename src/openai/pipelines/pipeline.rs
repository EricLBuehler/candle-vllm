        ...
        let (models, devices, config, sep_style, mtp_heads) = if gguf {
            ...
            let results: Vec<_> = device_ids
                ...
                .map(|(rank, dev_id)| {
                    ...
                    // Safetensors weights are mmap-backed and CUDA kernels
                    // may still be using the loading stream when the model
                    // becomes visible to the serving loop. Make completion
                    // explicit before returning this device/model pair.
                    #[cfg(feature = "cuda")]
                    device.synchronize()?;
