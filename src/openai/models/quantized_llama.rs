...
impl GGUFLLaMa {
    ...
    pub fn from_gguf(
        ...
        let tok_embeddings_q = vb.get_no_shape("token_embd.weight")?;
        let vocab_size = tok_embeddings_q.shape().dims()[0];
        ...
        for layer_idx in 0..block_count {
            ...
            let mlp_or_moe = if n_expert <= 1 {
                ...
                let feed_forward_gate_inp = prefix_vb.get_no_shape("ffn_gate_inp.weight")?;
                let mut experts = Vec::with_capacity(n_expert);
                for i in 0..n_expert {
                    let feed_forward_w1 =
                        prefix_vb.get_no_shape(&format!("ffn_gate.{i}.weight"))?;
                    let feed_forward_w2 =
                        prefix_vb.get_no_shape(&format!("ffn_down.{i}.weight"))?;
                    let feed_forward_w3 = prefix_vb.get_no_shape(&format!("ffn_up.{i}.weight"))?;
                    experts.push(Mlp {
                        feed_forward_w1: QMatMul::from_arc(feed_forward_w1)?,
                        feed_forward_w2: QMatMul::from_arc(feed_forward_w2)?,
                        feed_forward_w3: QMatMul::from_arc(feed_forward_w3)?,
                ...
                MlpOrMoe::MoE {
                    n_expert_used,
                    feed_forward_gate_inp: QMatMul::from_arc(feed_forward_gate_inp)?,
                    experts,
                }
            };
