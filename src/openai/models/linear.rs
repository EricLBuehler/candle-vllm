...
pub fn qmatmul_forward(linear: &QMatMul, x: &Tensor) -> Result<Tensor> {
    ...
    if let QMatMul::QTensor(weight) = linear {
        if is_iq_gguf_dtype(weight.dtype()) {
            ...
            return gguf_iq_matmul(&x, weight);
        }
    }
    linear.forward(x)
}
...
impl QLinear {
    ...
    /// Materialize a quantized weight for the vocabulary-padding path.
    pub fn dequantize(&self) -> Result<Tensor> {
        self.inner.dequantize_f16()
    }
...
fn load_ln_fp8_with_hints(
    ...
    // Some FP8 exporters store a per-tensor weight scale as a scalar rather
    // than a 2-D block-scale matrix. Materialize it as the local scale matrix
    // so it remains valid when the weight is tensor-parallel sharded.
    fn load_scale(
        vb: &VarBuilder,
        name: &str,
        scale_dim0: usize,
