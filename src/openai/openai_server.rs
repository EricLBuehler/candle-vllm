...
async fn chat_completions(
    ...
    if request.stream.is_some_and(|x| x) {
        ...
        let _ = thread::spawn(move || {
            let mut model = data.model.lock().unwrap();
            let model_res = model.generate(token_ids, request_id, created, sampling_params, request.logprobs.unwrap_or(false));
            if model_res.is_err() {
    ...
    let result = {
        let mut model = data.model.lock().unwrap();
        let model_res = model.generate(token_ids, request_id.clone(), created, sampling_params, request.logprobs.unwrap_or(false));
        if model_res.is_err() {
