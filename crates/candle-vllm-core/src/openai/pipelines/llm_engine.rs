...
impl LLMEngine {
    ...
    fn get_stream_response(
        ...
        if let Some(ref text_chunk) = content {
            ...
            let accumulated_text = accumulated.entry(request_id.clone()).or_insert_with(String::new);
            ...
            // Check if accumulated text might contain tool calls
            if parser.might_contain_tool_call(accumulated_text) {
                ...
                if let Some(tool_calls) = parsed.tool_calls() {
                    ...
                    let existing_calls = parsed_tool_calls_map.entry(request_id.clone()).or_insert_with(Vec::new);
                    
                    // Generate deltas for new tool calls
                    ...
                    for (idx, tool_call) in tool_calls.iter().enumerate() {
                        ...
                        if is_new {
                            // First chunk: send id, type, name, and initial arguments
                            ...
                            // Update: send only arguments delta if changed
        ...
        // If finished, clean up accumulated text
        ...
        loop {
            ...
            if rank == 0 {
                ...
                if !scheduler_outputs.ignored_seq_groups.is_empty() {
                    // Sequences that exceeded capacity are already marked as FinishedIgnored
                    // They will be handled in the normal completion flow
                    tracing::warn!(
                        "Ignored {} sequence groups that exceeded capacity",
                        scheduler_outputs.ignored_seq_groups.len()
                    );
                }
