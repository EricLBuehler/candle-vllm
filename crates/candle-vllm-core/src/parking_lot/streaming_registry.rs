...
//! streaming token channels, allowing the ResourcePool to return serializable
...
///
/// This registry allows the ResourcePool to return serializable mailbox keys
...
impl StreamingRegistry {
    ...
    ///
    /// The key can be used to retrieve the channel later.
    pub fn register(
        &self,
        channel_key: String,
        request_id: String,
        ...
        info!(
            "📝 REGISTRY: Registered streaming channel - request_id={}, key={}",
    ...
    pub fn retrieve(
        ...
        if let Some(entry) = channels.get(channel_key) {
            debug!(
                "📤 REGISTRY: Retrieved streaming channel - request_id={}, key={}",
                entry.request_id, channel_key
            );
            Some(entry.receiver.clone())
        } else {
            warn!(
                "⚠️ REGISTRY: Streaming channel not found - key={}",
                channel_key
            );
            None
        }
    ...
    ///
    /// This should be called when streaming is complete to free memory.
    pub fn remove(&self, channel_key: &str) -> bool {
        let removed = self.channels.write().remove(channel_key);
        if let Some(entry) = removed {
            info!(
                "🗑️ REGISTRY: Removed streaming channel - request_id={}, key={}",
                entry.request_id, channel_key
            );
            true
        } else {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        channels.retain(|key, entry| {
            ...
            if age > self.retention_duration {
                warn!(
                    "🧹 REGISTRY: Cleaning up expired streaming channel - request_id={}, key={}, age={}s",
                    entry.request_id,
                    key,
                    age.as_secs()
                );
        ...
        if removed > 0 {
            info!(
                "♻️ REGISTRY: Cleanup complete - removed {} expired channels",
                removed
            );
        }
        removed
    ...
    ///
    /// Returns a tokio task handle that can be joined.
...
mod tests {
    ...
    fn test_register_and_retrieve() {
        ...
        // Should be able to receive through retrieved channel
