...
/// Get the MCP proxy port from environment variable or default to 3000
fn mcp_proxy_port() -> u16 {
    std::env::var("MCP_PROXY_PORT")
        .ok()
    ...
    pub fn load(path: &str) -> anyhow::Result<Self> {
        ...
        if let Some(servers_array) = json.get("servers").and_then(|s| s.as_array()) {
            let servers: Vec<McpServerDefinition> =
                serde_json::from_value(serde_json::Value::Array(servers_array.clone()))?;
        ...
        if let Some(mcp_servers) = json.get("mcpServers").and_then(|s| s.as_object()) {
            ...
            for (name, entry_value) in mcp_servers {
                ...
                if let Some(url) = entry.url {
                    ...
                    // Use MCP_PROXY_PORT env var or default to port 3000
                    let proxy_port = mcp_proxy_port();
                    let url = format!("http://localhost:{}/{}", proxy_port, name);
                    tracing::warn!(
