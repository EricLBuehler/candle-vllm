...
- KV缓存配置、采样参数等
  ...
  <details>
    ...
    `--kv-fraction` 提供一个更轻量的自动模式。不显式指定时，默认值为 `0.6`。模型加载完成后，candle-vllm 会探测每张已加载的 CUDA 或 Metal 设备，并按以下公式计算 KV Cache 预算：

    ```
    cache_budget = kv_fraction * remaining_gpu_memory - workspace_reserve
    ```

    其中 workspace_reserve 是根据模型配置和编译特性自动计算的运行时缓冲区预留（包括 FlashInfer 工作空间、CUTLASS 工作空间、MoE 激活池、flash split-K 缓冲区和瞬态激活）。多卡场景下，会取所有 rank 中最小的结果作为每个 rank 的 KV Cache 预算。例如：

    ```
    candle-vllm --w /home/Qwen3-Coder-30B-A3B-Instruct-FP8 --d 0,1 --kv-fraction 0.6
    ```

    当你需要显式固定缓存预算时，用 `--mem`。当你希望服务根据模型加载后的可用显存自动调整时，用 `--kv-fraction`。
